...
export default function AdminRoomReservations() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get('/reservations/room', { params });
  ...
  const updateStatus = async (id, status) => {
    setUpdatingId(id);
    try {
      await api.put(`/reservations/room/${id}`, { status });
  ...
  const handleDelete = async id => {
    ...
    setDeletingId(id);
    try {
      await api.delete(`/reservations/room/${id}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-serif text-2xl sm:text-3xl text-charcoal-900 font-light">Room Reservations</h1>
          <p className="font-sans text-xs text-charcoal-400 mt-0.5">{reservations.length} records</p>
        </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {STATUSES.map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`font-sans text-[10px] tracking-[0.12em] uppercase px-3 py-1.5 transition-all ${filterStatus === s ? 'bg-gold-gradient text-charcoal-900 font-medium' : 'bg-cream-100 text-charcoal-500 hover:bg-cream-200'}`}>
            {s}
          </button>
        ))}
      </div>
            ...
            <div key={r._id} className="bg-white border border-cream-200 p-4 hover:border-gold-300 transition-colors">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-gold-gradient flex items-center justify-center font-serif text-charcoal-900 font-semibold text-base flex-shrink-0">{r.name.charAt(0)}</div>
                  <div>
                    ...
                    <p className="font-sans text-xs text-charcoal-400 mt-0.5">🏨 {r.roomType} · 👥 {r.guests} guests</p>
                    <p className="font-sans text-xs text-charcoal-400">📅 {r.checkIn} → {r.checkOut} · 📞 {r.phone}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => setExpanded(expanded === r._id ? null : r._id)}
                    className="font-sans text-[10px] tracking-wide uppercase px-3 py-1.5 bg-cream-100 text-charcoal-600 hover:bg-cream-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['Pending','Confirmed','Cancelled','Completed'].map(s => (
                      <button key={s} onClick={() => updateStatus(r._id, s)} disabled={r.status === s || updatingId === r._id}
                        className={`font-sans text-[10px] tracking-wide uppercase px-3 py-1.5 border transition-colors disabled:opacity-40 ${r.status === s ? 'bg-charcoal-900 text-cream-50 border-charcoal-900' : 'border-charcoal-200 text-charcoal-600 hover:bg-cream-100'}`}>
                        {s}
                      </button>
                    ))}
                  </div>

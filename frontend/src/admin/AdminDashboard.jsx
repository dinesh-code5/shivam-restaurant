...
function StatCard({ label, value, sub, icon, gold = false, link }) {
  const inner = (
    <div className={`p-5 border transition-shadow hover:shadow-md ${gold ? 'bg-gold-gradient border-transparent' : 'bg-white border-cream-200'}`}>
      <div className="flex items-start justify-between mb-3">
        <span className="text-2xl">{icon}</span>
        {link && <span className={`font-sans text-[10px] ${gold ? 'text-charcoal-700' : 'text-charcoal-300'}`}>→</span>}
      </div>
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-7">
      ...
      {/* Alerts */}
      {(data.pendingTableReservations > 0 || data.pendingRoomReservations > 0 || data.unreadMessages > 0) && (
